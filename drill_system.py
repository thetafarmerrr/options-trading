...
每次训练计时+记分，数据存 drill_state/history.json。吾
"""
...
def generate_chain_segment(dirty=False):
    """生成一段模拟期权链（看跌）。
    dirty=False: 干净链，0或1个异常，练识别能力。
    dirty=True:  真实链，0-3个混合异常，练区分真假能力。
    """
    ...
    if dirty:
        # ── 真实模式：注入 0-3 个混合异常 ──
        tradeable_strikes = []  # 记录哪些是可交易的倒挂
        all_anomalies = []      # 所有异常（含假的）
        ...
        # ── 干净模式：0 或 1 个异常 ──
        anomaly_type = random.choice(["inversion", "zero_bid", "wide_spread", "none"])

        if anomaly_type == "inversion":
            idx = random.randint(2, len(strikes) - 2)
            prices[idx]["theo"] = round(prices[idx-1]["theo"] * random.uniform(0.5, 0.8), 2)
            prices[idx]["bid"] = round(prices[idx-1]["bid"] * random.uniform(0.5, 0.8), 2) if prices[idx-1]["bid"] > 0 else 0
            prices[idx]["ask"] = round(prices[idx-1]["ask"] * random.uniform(0.7, 0.9), 2)
            anomaly_detail = f"倒挂: 行权价{strikes[idx]}更贵但反而更便宜"
        elif anomaly_type == "zero_bid":
            idx = random.randint(2, len(strikes) - 2)
            prices[idx]["bid"] = 0.0
            anomaly_detail = "零买价: 此合约无法平仓"
        elif anomaly_type == "wide_spread":
            idx = random.randint(2, len(strikes) - 2)
            prices[idx]["bid"] = round(prices[idx]["theo"] * 0.3, 2)
            prices[idx]["ask"] = round(prices[idx]["theo"] * 2.5, 2)
            anomaly_detail = "价差过宽: 滑点吃掉利润"
        else:
            idx = None
            anomaly_detail = "无异常（正常链）"

        return {
            "product": product, "futures": fut, "strikes": strikes,
            "prices": [{**p} for p in prices],
            "mode": "clean",
            "anomaly_type": anomaly_type, "anomaly_index": idx,
            "anomaly_detail": anomaly_detail,
        }


def run_drill_a(quick=False, hard=False):
    """训练 A：链面扫异常。hard=False 初级(0-1异常), hard=True 高级(真实链,辨真假)"""
    mode_label = "高级·辨真假" if hard else "初级·识异常"
    print(f"\n{'─'*60}")
    print(f"  训练 A — 链面扫异常 [{mode_label}]")
    if hard:
        print(f"  规则：真实市场链面，可能有多处异常")
        print(f"  找出真正可交易的倒挂（有买价 + 价差合理）")
        print(f"  忽略：零买价/价差过宽/假倒挂（那些是流动性垃圾，不是机会）")
        print(f"  目标：正确率 > 85%，不漏真机会，不误报假机会")
    else:
        print(f"  规则：找出异常（倒挂/零买价/价差过宽/无异常）")
        print(f"  目标：10秒/题，正确率 > 95%")
    print(f"{'─'*60}")

    n_rounds = 5 if quick else (10 if hard else 15)
    score = 0
    total_time = 0
    correct = 0

    for round_num in range(1, n_rounds + 1):
        chain = generate_chain_segment(dirty=hard)

        # 显示链面
        print(f"\n  [{round_num}/{n_rounds}] {chain['product']} 期货≈{chain['futures']}")
        print(f"  {'行权价':>6} │ {'P 买价':>8} {'P 卖价':>8}")
        print(f"  {'─'*6}─┼─{'─'*16}")

        for i, (s, p) in enumerate(zip(chain["strikes"], chain["prices"])):
            bid_str = f"{p['bid']:.2f}" if p['bid'] > 0 else "0.00 ⚠"
            ask_str = f"{p['ask']:.2f}"
            print(f"  {s:>6} │ {bid_str:>8} {ask_str:>8}")
        ...
        if hard:
            ...
            # 显示全部异常供学习
            if chain["all_anomalies"] and not (not false_positives and not misses):
                print(f"  📝 全部异常解析:")
            ...
            # 初级模式：原有逻辑
            try:
                answer = input(f"  → 哪一行异常？输入行权价或 '无': ").strip()
            except (EOFError, KeyboardInterrupt):
                print(f"\n  训练中断。已保存当前进度。")
                return
            elapsed = (time.time() - t_start) * 1000
            ...
            is_anomaly = chain["anomaly_type"] != "none"
            if is_anomaly:
                expected = str(chain["strikes"][chain["anomaly_index"]])
                is_correct = (answer == expected)
            else:
                is_correct = (answer.lower() in ["无", "none", "n", "no"])
            ...
            if is_correct:
                ...
                if elapsed < 5000:
                    ...
                    score += 1
                print(f"  ✅ 正确！({elapsed/1000:.1f}s) → {chain['anomaly_detail']}")
            else:
                if is_anomaly:
                    print(f"  ❌ 错了。异常在行权价 {expected}：{chain['anomaly_detail']}")
                else:
                    print(f"  ❌ 错了。这条链没有异常。")
    ...
    print(f"\n  {'─'*40}")
    print(f"  训练A [{mode_label}] 完成: {correct}/{n_rounds} 正确 ({accuracy:.0f}%)")
    print(f"  平均用时: {avg_time/1000:.1f}s/题  得分: {score}")
    if hard:
        rating = "⭐ 目光如炬！" if accuracy >= 85 else ("👍 继续磨" if accuracy >= 65 else "🐢 多看真实链面")
    else:
        rating = "⭐ 太快了！" if avg_time < 5000 else ("👍 不错" if avg_time < 10000 else "🐢 再练练速度")
    print(f"  {rating}")
    print()
...
def main():
    ...
    parser.add_argument('--quick', action='store_true', help='快速模式: 题量缩减')
    parser.add_argument('--hard', action='store_true', help='高级模式: 训练A启用真实链(多异常辨真假)')
    ...
    if module == "A":
        run_drill_a(quick=args.quick, hard=args.hard)
    elif module == "B":
