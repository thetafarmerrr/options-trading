...
| 超额峰度 | Excess Kurtosis | 分布比正态"尖峰肥尾"的程度。9/10 Sinclair Ch5 空跨式实测 **12.2**（≈正态 100 倍以上）→ 极端跳空发生频率远超正态假设。**肥尾 = 卖方被"罕见但惨"的那几笔决定生死**，不能只看胜率 |
| 赢亏比 | Win/Loss Size | 平均赢的单 ÷ 平均输的单。Sinclair Ch5 空跨式 = **0.69**（配 64% 胜率）→ 赢得频繁但赢得小。**胜率与赢亏比必须一起看**：64% 胜率单独看像稳赚，配上 0.69 + 偏度 −2.1 才是真实画像 |
| 波动率风险溢价（VRP）| Variance Risk Premium | IV − HV。**正** = 保险卖得贵 = 卖方收溢价（S1 赚的就是这个）；**负** = IV < HV = 保险卖便宜了 = **买方有利、卖方负 EV**。9/10 lc 实测 ≈ **−8.0pp**（对照 rm −3.3pp 已被规则毙） |
| 涨跌停板（商品期货）| Limit Up / Limit Down | 不是"价格到顶"，是**流动性瞬间消失**：停板时挂单撮合不了 → **止损单挂不出去**；反向持仓被追保；次日开板常跳空。**对裸方向持仓是结构性风险**（买方正 gamma 能被跳空送礼，卖方/裸多被跳空打）；对买方跨式/宽跨式反而中性偏好（最大损失已在权利金锁定） |
...
| 错失恐惧 | FOMO (Fear of Missing Out) | 怕踏空/怕错过 → 追单、硬找信号、换仓。是情绪不是信号。本质=注意力从「我的规则说什么」漂移到「市场正在发生什么」；索取心态 vs 显身心态。系统挡板：禁止#5（空仓也是仓位）、四层独立讲清、纸面信号逐条过、D 门零 override（9/1 记） |
...
| 倒挂（近月>远月）| Inversion / Backwardation | 近月 IV>远月 IV = 近端被事件/恐慌买高（近端保险定价）。卖方不做等结构修复；修复判据 = 近月 IV 跌破远月回 contango（8/21 豆粕案例：天气窗口+近月压力驱动）|
| 参考腿 | Reference Leg | 验证倒挂真实性的紧邻低档。**三层判据（8/24 定）**：①无效档（bid≤0 或价差>50%）零信息直接跳过 ②有效宽档（价差≤50% 但超干净线）须纳入比较、阻断跨腿 ③干净参考腿（<10元→价差<30%、10-30元→<20%、≥30元→<15%）。**倒挂存在≠可交易**——跳过中间档=赌错价。50% 有效线为教学参数无实证来源（8/15 定案，非实盘依据） |
| 死档 vs 活但脏 | Dead vs Alive-but-dirty Quote | 死档（零买价/价差>50%）=零信息可跳过；活但脏（有 bid 但价差 30-50%）=真实报价须纳入比较，阻断跨腿（漏报代价小，误报进垃圾流动性代价大）|
| 倒挂净空间 | Mispricing minus Slippage | 机会净值=错误定价幅度−成交滑点。倒挂本质是错误定价；参考腿管真实性，本档价差管净空间，错误定价>滑点才是真机会（8/21 沪金 782：低估 34% > 滑点 10%）|
| 错价 vs 补偿 | Misprice vs Compensation | 卖方只在 IV 贵是**错价**时收钱（贵得没道理→收得到溢价）；贵是**补偿**时不做（真实事件/结构风险落在合约生命期→收的是风险补偿非错价）。判别问句：这贵背后有没有真实、会落在合约窗内的风险？有=补偿不做；无=错价才收。法则①倒挂/事件层/高分位全是这一句的不同长相（9/8 记，cf sr 台风 + ma 倒挂两 EXEC 同源死因）|
...
| 组合保证金 | Portfolio Margin | 按组合整体风险算保证金（比 Reg-T 宽松）。**价差组合 = 行权价差×乘数 − 净权利金**（9/1 实测 cf2701 P16400/16200 ≈655 元）；单腿卖方 = 权利金 + max(期货保证金−虚值额, 0.5×期货保证金)（同腿虚拟盘收 24945，差 7 倍）。**组合单开关**：只有交易所价差指令（组合单）才享受组合保证金，自设套利（两腿分挂）不享受——实盘首单必验冻结价 |
...
**当期实况 · 2026-09-10 查证**（NOAA/WMO 9/3 口径）：9-11 月发生概率 **100%**，GFDL SPEAR 集合 **30/30** 成员预报，为有记录以来最强；Niño-3.4 +1.4°C。→ 用户在 9/10 讨论中"预期强"的说法**成立**（教练当时没查证就质疑，错）。
**但必须分清**：厄尔尼诺对 c/m/rm 是**价格方向**因素（南美增产 → 偏空），**不是 IV 因素**——"理由正确但接错因变量"是 9/10 那节课的核心。且概率 100% 的事件**没有边际信息**（已被定价），拿它推方向 = 拿已知推已知。
