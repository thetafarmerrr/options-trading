...
| 1 | 9:15-9:30 | **开盘决策窗口** | **9:15 iv_collector 自动采完 → 执行 `python3 -m tools.scanner.main` → 用新输出决策**（8/14 教训：盘前盘口是脏数据源，所以盘前已不跑 scanner，决策只用 9:15 这次输出）。有 [EXEC]→复制出来，进六步进场（monitoring-rules.md 第三节）；无信号→记干旱天数。**这是真正的决策锚点** |
