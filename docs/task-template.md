...
| 2 | **IV 早盘·查看** | 9:30 自动采集已跑（launchd）。`tail -80 data/iv_collector_scheduler.log` 看早盘输出。每个品种五条打分：≥3 条有利=可做卖方；≥3 条不利=今天不交易。买方视角：哪些品种 IV 分位 <30%→买方黄金窗口。⚠️ 若日志无今日条目→电脑当时休眠，手动补：`python3 tools/iv_collector.py --window morning` |
| 2b | **IV 收盘·对比** | 14:56 自动采集已跑（launchd）。`tail -40 data/iv_collector_scheduler.log` 看下午输出。核心动作：**早盘 vs 下午对比**——IV 比早上贵了还是便宜了？哪个品种 IV-HV 价差方向翻转了？写进 journal 一句（例："甲醇早 IV 10.8%→收 IV 10.2%，IV-HV 价差 -20.9%，折价未修复"）。⚠️ 若日志无下午条目→手动补：`python3 tools/iv_collector.py --window afternoon` |
