...
| 5 | **影响力** | 有料→教练写入 `data/fragments.json`。你跑 `python3 tools/post_fragment.py` 发送。首次需配凭证：Twitter Developer 申请 + Reddit Script App → 填入 `tools/fragment_credentials.json`。没料→过。例：甲醇 ATM +14% 但 call wing 折价——卖哪条腿看偏度 |
