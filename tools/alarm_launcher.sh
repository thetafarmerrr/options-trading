#!/bin/bash
# 早起闹钟 wrapper —— 由 ~/Library/LaunchAgents/com.goldopt.alarm.plist 在每天 7:10 调用
#（9/24 从 8:10 改到 7:10）
...
# 要改时间：改 plist 的 StartCalendarInterval，重载即可（见收尾说明）
{
    ...
    for i in 1 2 3 4 5; do
        say -v Meijia "早上七点十分，起床了。拉开窗帘见光，今天不要睡午觉。"
        sleep 4
    done
    # 弹窗通知（声音 Glass）
    osascript -e 'display notification "7:10 起床锚：拉开窗帘见光 10 分钟，午觉只许 14:00 前 20 分钟" with title "🌅 起床闹钟" sound name "Glass"'
