...
class Reporter:
    ...
    def print_iv_hv_panel(panel_data: list):
        ...
        # 标签诚实性（9/12 修）：此列算法是 ivs[-1] − ivs[-6] = 在扁平行列表里**往回数 5 条采集**，
        # 不是往回 5 个交易日（iv_history 约 2 条/天 → 实际跨度约 2.5 个交易日）。
        # 原标签写「近 5 日变动」，违反数据诚实第 1 条（字段名不得伪装）。此处只改标签，
        # **不改算法**——改算法＝改口径，属门级，走质疑轮（见 journal 9/12 R7）。
        print(f"  └─ 5dΔ = 最新 IV − 往回第 5 条采集（约 2.5 个交易日，**非 5 个交易日**）。数据点 = 该品种最新健康 IV 的日期时间（旧日期/⚠️失真 = 盘前快照，决策前 9:15 重跑）\n")
